...
核心计算部分纯C++，只有渲染、流程控制、线程部分使用OBJ-C，移植应该没什么问题。
...
最后，建议本控件初始化过程在主线程中实现。

因为初始化过程中，使用了系统绘图上下文来重采样图片（图方便偷个懒）。所以在子线程中来初始化本控件有可能会出现与软件其他与图形上下文相关的行为冲突，带来错误。
...
进行三次涂抹，用模糊原图按区块顺序加权平均一次，让水彩效果产生立体感，接近原图。
